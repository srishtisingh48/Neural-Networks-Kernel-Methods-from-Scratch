...
def calculate_centers(
    ...
    for i in range(num_centers):
        if len(data[classifications == i]) > 0:
            centers[i] = data[classifications == i].mean(axis=0)
    return centers
...
def cluster_data(data: np.ndarray, centers: np.ndarray) -> np.ndarray:
    ...
    """
    return np.argmin(np.linalg.norm(data[:, np.newaxis] - centers, axis=2), axis=1)
...
def lloyd_algorithm(
    ...
    errors = []
    while True:
        new_centers = calculate_centers(data, cluster_data(data, centers), num_centers)
        errors.append(calculate_error(data, new_centers))
